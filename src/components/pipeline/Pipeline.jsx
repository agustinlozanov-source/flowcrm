...
import AssignProductModal from '@/components/pipeline/AssignProductModal'
import { Search, Plus, Pencil, TrendingUp, DollarSign, Lock } from 'lucide-react'
...
export default function Pipeline() {
  ...
  const [assignProductLead, setAssignProductLead] = useState(null)
  ...
  // Active pipeline config (for handoff bound days)
  const activePipeline = pipelines.find(p => p.id === activePipelineId)
  const handoffBoundDays = activePipeline?.handoffBoundDays || 3
  const handoffBoundActive = activePipeline?.handoffBoundActive || false
  ...
  const handleCreateStage = async () => {
    ...
    try {
      ...
      setNewStageName(''); setNewStageColor('#6366f1'); setShowNewStage(false)
      toast.success('Etapa creada')
    } catch { toast.error('Error al crear la etapa') }
  ...
  const filteredSystemLeadsByStage = Object.keys(SYSTEM_STAGES).reduce((acc, key) => {
    let stageLeads = leadsBySystemStage[key] || []
    if (sourceFilter !== 'all') stageLeads = stageLeads.filter(l => l.source === sourceFilter)
    if (search.trim()) {
      const q = search.toLowerCase()
      stageLeads = stageLeads.filter(l =>
  ...
  const handleDragEnd = useCallback(async ({ active, over }) => {
    ...
    if (lead.stageId !== targetStageId) {
      try { await moveLead(lead.id, targetStageId) }
      catch { toast.error('Error al mover el lead') }
    }
  }, [leads, moveLead])
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="bg-surface border-b border-black/[0.08] px-5 h-[68px] flex items-center gap-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {pipelineStats.potential > 0 && (
            <span className="text-[11px] font-semibold bg-blue-50 text-accent-blue px-2.5 py-1 rounded-full flex items-center gap-1">
              <TrendingUp size={10} /> {fmt(pipelineStats.potential)} potencial
            </span>
          )}
          {pipelineStats.secure > 0 && (
            <span className="text-[11px] font-semibold bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-full flex items-center gap-1">
              <Lock size={10} /> {fmt(pipelineStats.secure)} seguro
            </span>
          )}
          {pipelineStats.closed > 0 && (
            <span className="text-[11px] font-semibold bg-green-50 text-green-600 px-2.5 py-1 rounded-full flex items-center gap-1">
          ...
          <div className="flex gap-4 h-full p-5 w-max">
            ...
            {stages.map(stage => (
              <KanbanColumn
                ...
                onAddLead={(stageId) => { setDefaultStageId(stageId); setShowNewLead(true) }}
              />
            ))}
            ...
            <div className="w-[260px] min-w-[260px] flex flex-col gap-2 pt-1">
              {!showNewStage ? (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-[10px] border border-dashed border-black/[0.14] text-[12.5px] text-tertiary hover:text-secondary hover:border-black/25 transition-all w-full">
                  <Plus size={14} strokeWidth={2.5} /> Nueva etapa
                </button>
              ) : (
                <div className="bg-surface border border-black/[0.08] rounded-[12px] p-3 flex flex-col gap-3 shadow-sm">
                  <input ref={stageInputRef} value={newStageName}
                  ...
                  <div className="flex gap-2">
                    <button onClick={handleCreateStage} disabled={!newStageName.trim() || savingStage}
                      className="btn-primary text-[12px] py-1.5 px-3 flex-1 disabled:opacity-40">
                      {savingStage ? 'Guardando...' : 'Crear etapa'}
                    </button>
                    <button onClick={() => { setShowNewStage(false); setNewStageName(''); setNewStageColor('#6366f1') }}
            ...
            {/* Divider between configurable and system stages */}
            <div className="flex flex-col justify-start pt-1">
              <div className="w-px bg-black/[0.08] h-full mx-2" />
            </div>

            {/* System stage columns */}
            {Object.values(SYSTEM_STAGES).map(systemStage => (
              <SystemStageColumn
                key={systemStage.id}
                systemStage={systemStage}
                leads={filteredSystemLeadsByStage[systemStage.id] || []}
                onLeadClick={(lead) => setSelectedLead(lead)}
                onAssignProduct={(lead) => setAssignProductLead(lead)}
                handoffBoundDays={handoffBoundActive ? handoffBoundDays : null}
              />
            ))}
      ...
      {selectedLead && (
        <LeadDrawer
          lead={leads.find(l => l.id === selectedLead.id)
            || systemLeads.find(l => l.id === selectedLead.id)
            || selectedLead}
      ...
      {assignProductLead && (
        <AssignProductModal
          lead={assignProductLead}
          onClose={() => setAssignProductLead(null)}
          onSave={async ({ productId, productName, price, currency }) => {
            await updateLead(assignProductLead.id, { productId, value: price })
            toast.success('Producto asignado')
            setAssignProductLead(null)
          }} />
      )}
    </div>
  )
