...
import {
  ...
  Copy, Check, Link2, Shield, ChevronDown, ChevronUp,
  ZoomIn, ZoomOut, Maximize2, AlertTriangle, Star,
  TrendingUp, UserCheck, UserX, Mail, Info
} from 'lucide-react'
...
// ─── VENDOR MODAL (internal team member) ─────────────────────────
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-[18px] shadow-[0_24px_80px_rgba(0,0,0,0.18)] w-full max-w-lg border border-black/[0.08] flex flex-col max-h-[90vh]">
        ...
        {/* Header */}
        <div className="flex items-start gap-4 px-6 py-5 border-b border-black/[0.06] flex-shrink-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11.5px] text-secondary mt-0.5">
              Usuario interno de tu equipo — comparte tu CRM
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
          ...
          {activeSection === 'info' && (
            <>
                  ...
                  {Object.entries(ROLE_CONFIG).map(([key, cfg]) => (
                    <button key={key} type="button" onClick={() => handleRoleChange(key)}
                      className={clsx('flex-1 py-2.5 rounded-[10px] border text-[13px] font-semibold transition-all',
                        form.role === key ? 'border-primary bg-primary/[0.06] text-primary' : 'border-black/[0.1] text-secondary hover:border-black/[0.2]')}>
                      {cfg.label}
                    </button>
                  ))}
                </div>
              ...
              {/* Type */}
              <div>
                <label className="text-[11px] font-semibold text-secondary uppercase tracking-wide block mb-2">Tipo</label>
                  ...
                  {Object.entries(TYPE_CONFIG).map(([key, cfg]) => (
                    <button key={key} type="button" onClick={() => set('type', key)}
                      className={clsx('flex items-center gap-2.5 px-3 py-2 rounded-[8px] border text-left transition-all',
                        form.type === key ? 'border-primary/40 bg-primary/[0.04]' : 'border-black/[0.08] hover:border-black/[0.16]')}>
                      <div className={clsx('w-3 h-3 rounded-full border-2 flex-shrink-0',
                        form.type === key ? 'border-primary bg-primary' : 'border-black/20')} />
                      <span className="text-[12.5px] font-semibold text-primary">{cfg.label}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <>
              {form.role === 'admin' ? (
                <div className="flex items-start gap-3 p-4 bg-purple-50 border border-purple-200 rounded-[12px]">
                  ...
                  <p className="text-[12.5px] text-purple-700 leading-relaxed">
                    Los administradores tienen todos los permisos activos automáticamente.
                  </p>
                </div>
              ) : (
                <>
                  <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-200 rounded-[10px]">
                    ...
                    <p className="text-[11.5px] text-blue-700 leading-relaxed">
                      Por defecto los vendedores solo ven sus propios leads. Activa permisos adicionales aquí.
                    </p>
                  </div>
                    ...
                    <div key={group}>
                      ...
                      <div className="flex flex-col gap-1.5">
                        {Object.entries(PERMISSIONS)
                          ...
                          .map(([key, def]) => (
                            <div key={key} className="flex items-center justify-between px-3 py-2.5 rounded-[8px] border border-black/[0.06] bg-surface">
...
function DistributorInviteModal({ onClose, onCreate }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-[18px] shadow-[0_24px_80px_rgba(0,0,0,0.18)] w-full max-w-md border border-black/[0.08] p-6">
        ...
        <div className="flex items-start gap-4 mb-5">
          <div className="w-10 h-10 rounded-[10px] bg-green-50 flex items-center justify-center flex-shrink-0">
            <GitBranch size={18} className="text-green-600" />
          </div>
          <div className="flex-1">
            <h2 className="font-display font-bold text-[15px]">Invitar distribuidor</h2>
            <p className="text-[11.5px] text-secondary mt-0.5">Crea un link para que alguien se una a tu red</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-tertiary hover:bg-surface-2">
            <X size={15} />
          </button>
        ...
        {!invite ? (
          <>
            ...
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-[12px] mb-5">
              ...
              <div>
                <p className="text-[12.5px] text-amber-800 font-bold mb-1">
                  Esto NO es un vendedor de tu equipo
                </p>
                <p className="text-[11.5px] text-amber-700 leading-relaxed">
                  La persona que uses este link creará su <strong>propio CRM independiente</strong> y pagará su propia membresía. Quedará bajo tu genealogía y podrá a su vez invitar a otros distribuidores.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 mb-5">
              <p className="text-[11px] font-bold text-secondary uppercase tracking-wide">El distribuidor tendrá</p>
              {[
                'Su propio CRM con sus propios leads',
                'Su propio agente IA',
                'Su propio equipo de vendedores',
                'Acceso a la genealogía hacia abajo',
              ].map(item => (
                <div key={item} className="flex items-center gap-2 text-[12.5px] text-secondary">
          ...
          <>
            <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-[10px] mb-4">
              ...
              <p className="text-[12.5px] text-green-700 font-semibold">Link generado — expira en 7 días</p>
            </div>

            <div className="mb-4">
              <label className="text-[10px] font-bold text-secondary uppercase tracking-wide block mb-1.5">Código de invitación</label>
            ...
            <div className="flex gap-2">
              <a href={`https://wa.me/?text=Te invito a unirte a mi red en Flow Hub. Crea tu propio CRM aquí: ${invite.link}`}
              ...
              <a href={`mailto:?subject=Invitación a Flow Hub&body=Te invito a crear tu propio CRM en mi red. Regístrate aquí: ${invite.link}`}
...
// ─── VENDOR ROW ───────────────────────────────────────────────────
function VendorRow({ member, currentUserId, onEdit, onToggle, onDelete }) {
    ...
    <tr className="border-b border-black/[0.04] hover:bg-surface-2 transition-colors">
      ...
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3 text-[11.5px]">
          <span className="text-secondary">{member.stats?.activeLeads || 0} activos</span>
          <span className="text-green-600">{member.stats?.closedThisMonth || 0} cierres</span>
        </div>
      </td>
      <td className="px-5 py-3.5">
            ...
            <>
              <button onClick={() => onToggle(member)}
                className={clsx('w-7 h-7 rounded-lg flex items-center justify-center transition-colors',
                  member.active ? 'text-tertiary hover:bg-amber-50 hover:text-amber-500' : 'text-tertiary hover:bg-green-50 hover:text-green-500')}>
...
// ─── ORG CHART NODE ───────────────────────────────────────────────
function OrgNode({ node, onNodeClick }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div onClick={() => onNodeClick(node)}
        className="relative cursor-pointer group" style={{ width: 200 }}>
        <div className="rounded-[14px] border-2 border-black/[0.08] p-4 transition-all hover:shadow-card-md hover:border-primary/30 bg-surface">
          <div className="absolute top-0 left-0 right-0 h-1 rounded-t-[12px]" style={{ background: role.color }} />

          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-[12px] font-bold text-white flex-shrink-0"
              style={{ background: role.color }}>{initials}</div>
          ...
          <div className="flex items-center gap-3 text-[10.5px]">
            ...
            {node.children?.length > 0 && (
              <span className="text-blue-500 flex items-center gap-0.5">
                <GitBranch size={9} /> {node.children.length}
              </span>
            )}
          </div>
      ...
      {hasChildren && expanded && (
        <div className="flex flex-col items-center mt-6">
          ...
          {node.children.length > 1 ? (
            ...
            <OrgNode node={node.children[0]} onNodeClick={onNodeClick} />
          )}
        </div>
      )}
    </div>
...
function OrgChart({ tree, onNodeClick }) {
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="font-display font-bold text-base text-primary">Sin genealogía todavía</p>
          <p className="text-sm text-secondary mt-1 mb-4">Invita a tu primer distribuidor para comenzar tu red</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[radial-gradient(circle,_rgba(0,0,0,0.04)_1px,_transparent_1px)] bg-[size:24px_24px] cursor-grab active:cursor-grabbing"
      ...
      <div className="absolute bottom-4 left-4 flex flex-col gap-1.5 z-10">
        <button onClick={() => setScale(s => Math.min(s + 0.1, 2))}
          className="w-8 h-8 rounded-lg bg-surface border border-black/[0.1] flex items-center justify-center text-secondary hover:text-primary shadow-sm transition-all">
          <ZoomIn size={14} />
        </button>
        <button onClick={() => setScale(s => Math.max(s - 0.1, 0.3))}
          className="w-8 h-8 rounded-lg bg-surface border border-black/[0.1] flex items-center justify-center text-secondary hover:text-primary shadow-sm transition-all">
          <ZoomOut size={14} />
        </button>
        <button onClick={() => { setScale(0.9); setPosition({ x: 0, y: 20 }) }}
          className="w-8 h-8 rounded-lg bg-surface border border-black/[0.1] flex items-center justify-center text-secondary hover:text-primary shadow-sm transition-all">
          <Maximize2 size={14} />
        </button>
      </div>
...
// ─── NODE DETAIL PANEL ────────────────────────────────────────────
function NodeDetailPanel({ node, onClose, onEdit, onInvite }) {
  const role = ROLE_CONFIG[node.role] || ROLE_CONFIG.seller
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            { label: 'Red directa',   value: node.children?.length || 0, color: 'text-blue-600' },
          ].map(s => (
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => onInvite(node)}
            className="w-full flex items-center justify-center gap-1.5 py-2 rounded-[8px] bg-green-50 border border-green-200 text-green-700 text-[12px] font-semibold hover:bg-green-100 transition-colors">
            <GitBranch size={13} /> Invitar distribuidor bajo este nodo
          </button>
          <button onClick={() => onEdit(node)}
            className="w-full btn-secondary text-[12px] py-2 flex items-center justify-center gap-1.5">
...
export default function Team() {
  ...
  const handleSaveVendor = async (form) => {
    if (editMember) {
      await updateMember(editMember.id, {
        ...
        type: form.type, inRoundRobin: form.inRoundRobin,
      })
  ...
  const handleToggle = async (member) => {
    ...
    toast.success(member.active ? 'Vendedor desactivado' : 'Vendedor activado')
  }
  ...
  const handleCreateInvite = async (parentId) => {
    return await createInvite(parentId)
  }
    ...
    <div className="h-full flex flex-col overflow-hidden">
        ...
        {[
          {
            ...
            icon: Users,
            label: 'Vendedores',
            desc: 'Tu equipo interno',
          },
          {
            id: 'genealogy',
            icon: GitBranch,
            label: 'Genealogía',
            desc: 'Tu red de distribuidores',
          },
        ].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            <tab.icon size={14} />
            <span>{tab.label}</span>
            <span className={clsx('text-[10px] font-normal ml-0.5',
              activeTab === tab.id ? 'text-primary/60' : 'text-tertiary')}>
              — {tab.desc}
            </span>
          </button>
        ))}
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === 'members' && (
          <div className="h-full flex flex-col overflow-hidden">
            ...
            {/* Members topbar */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-black/[0.06] bg-surface flex-shrink-0">
              <div>
                <p className="text-[12.5px] font-semibold text-primary">Vendedores internos</p>
                <p className="text-[11px] text-secondary">
                  Comparten tu CRM — consumen slots de tu plan
                </p>
              </div>
            ...
            <div className="flex-1 overflow-auto">
              {filteredMembers.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-3">
                  ...
                  <p className="font-display font-bold text-lg text-primary">Sin vendedores todavía</p>
                ...
                <table className="w-full">
                  <thead className="sticky top-0 bg-surface border-b border-black/[0.08] z-10">
                    <tr>
                      {['Vendedor', 'Rol', 'Tipo', 'Actividad', 'Estado', 'Se unió', 'Acciones'].map(h => (
                  ...
                  <tbody>
                    {filteredMembers.map(member => (
                      <VendorRow key={member.id} member={member}
        ...
        {activeTab === 'genealogy' && (
          <div className="h-full flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-black/[0.06] bg-surface flex-shrink-0">
              <div>
                <p className="text-[12.5px] font-semibold text-primary">Red de distribuidores</p>
                <p className="text-[11px] text-secondary">
                  Cada nodo es un tenant independiente con su propio CRM
                </p>
              </div>
              <button
                onClick={() => setShowDistributorModal(true)}
                className="flex items-center gap-2 px-4 py-2 rounded-[10px] bg-green-500 hover:bg-green-600 text-white text-[12.5px] font-semibold transition-colors">
                <GitBranch size={14} /> Invitar distribuidor
              </button>
            </div>
            ...
            <div className="flex-1 relative overflow-hidden">
              ...
              {selectedNode && (
                <NodeDetailPanel
                  ...
                  onInvite={() => { setSelectedNode(null); setShowDistributorModal(true) }}
                />
              )}
              {!canSeeFullGenealogy && (
      ...
      {showVendorModal && (
        <VendorModal
          member={editMember}
          members={members}
          onClose={() => { setShowVendorModal(false); setEditMember(null) }}
          onSave={handleSaveVendor}
        />
      )}

      {showDistributorModal && (
        <DistributorInviteModal
          onClose={() => setShowDistributorModal(false)}
          onCreate={() => handleCreateInvite(user?.uid)}
        />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface rounded-[18px] w-full max-w-sm border border-black/[0.08] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.18)]">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-display font-bold text-[15px]">¿Eliminar vendedor?</h3>
            </div>
