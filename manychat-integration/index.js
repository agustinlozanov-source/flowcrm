...
function registerChannelOAuth(app, platform) {
  app.get(`/${platform}/connect`, async (req, res) => {
    ...
    try {
      ...
      // ── WhatsApp con número pre-asignado → usar Embedded Signup con FB SDK ──
      if (platform === 'whatsapp') {
        const whatsappData = integSnap.exists ? integSnap.data()?.whatsapp : null
        const metaPreverifiedId = whatsappData?.metaPreverifiedId
        const phoneNumber = whatsappData?.assignedNumber
        ...
        if (metaPreverifiedId) {
          // Obtener appId y configId desde Zernio
          const sdkRes = await axios.get('https://zernio.com/api/v1/connect/whatsapp/sdk-config', {
            headers: { Authorization: `Bearer ${process.env.ZERNIO_API_KEY}` }
          })
          const { appId, configId } = sdkRes.data
...
<body>
  <div class="card">
    ...
    ${phoneNumber ? `<div class="phone">${phoneNumber}</div><div class="badge">✅ Verificado · Sin OTP</div>` : ''}
  ...
  <script>
    ...
    var configId = ${JSON.stringify(configId)};
    var metaPreverifiedId = ${JSON.stringify(metaPreverifiedId)};
    var redirectBase = ${JSON.stringify(APP_URL + '/settings')};
    var railwayUrl = ${JSON.stringify(RAILWAY_URL)};
    ...
    function startSignup() {
      ...
      FB.login(function(response) {
        if (response.authResponse) {
          ...
          fetch(railwayUrl + '/whatsapp/embedded-signup', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ orgId: orgId, code: code })
          })
          ...
          .then(function(data) {
            if (data.success) {
              ...
              setTimeout(function() {
                if (window.opener) { window.opener.location.href = redirectBase + '?whatsapp=connected'; }
                window.close();
      ...
      // ── Flujo OAuth genérico (facebook, instagram, y whatsapp sin número asignado) ──
