...
// GET /whatsapp/connect?orgId=xxx — sirve la página HTML con el FB SDK para el embedded signup
app.get('/whatsapp/connect', async (req, res) => {
  const { orgId } = req.query
  if (!orgId) return res.status(400).send('<h2>Falta orgId</h2>')
  try {
    // Leer metaPreverifiedId y asignaciones de Firestore
    const integSnap = await db.collection('organizations').doc(orgId)
      .collection('settings').doc('integrations').get()
    ...
    const redirectBase = 'https://flowhubcrm.app/settings'
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card { background: white; border-radius: 18px; padding: 36px 32px;
      max-width: 420px; width: 90%; text-align: center;
      box-shadow: 0 4px 24px rgba(0,0,0,0.1); }
    .logo { font-size: 40px; margin-bottom: 16px; }
    h1 { font-size: 18px; font-weight: 800; color: #1a1a1a; margin-bottom: 8px; }
    p { font-size: 14px; color: #6e6e73; line-height: 1.5; margin-bottom: 24px; }
    .phone { font-size: 22px; font-weight: 800; color: #25d366; margin-bottom: 4px; }
    .badge { display: inline-flex; align-items: center; gap: 6px; font-size: 12px;
      color: #1a7f37; background: rgba(37,211,102,0.1); border: 1px solid rgba(37,211,102,0.25);
      border-radius: 20px; padding: 4px 12px; margin-bottom: 24px; font-weight: 600; }
    button { width: 100%; padding: 14px; background: #25d366; color: white;
    ...
    .status { margin-top: 16px; font-size: 13px; color: #6e6e73; min-height: 20px; }
    .error { color: #e53e3e; }
  </style>
</head>
<body>
  <div class="card">
    ...
    ${phoneNumber ? `<div class="phone">${phoneNumber}</div>
    <div class="badge">✅ Verificado · Sin OTP</div>` : '<p>Número asignado listo para conectar</p>'}
    <p>Inicia sesión con tu cuenta de Facebook Business, selecciona tu portfolio comercial y el número aparecerá pre-seleccionado.</p>
    <button id="connectBtn" onclick="startSignup()">Conectar con Meta →</button>
  ...
  <script>
    ...
    const metaPreverifiedId = ${JSON.stringify(metaPreverifiedId)}
    const redirectBase = ${JSON.stringify(redirectBase)}
    ...
    function startSignup() {
      ...
      var extras = { feature: 'whatsapp_embedded_signup' }
      if (metaPreverifiedId) {
        extras.setup = { phone_number_id: metaPreverifiedId }
      }

      FB.login(function(response) {
        if (response.authResponse) {
          var code = response.authResponse.code
          setStatus('Completando conexión...')
          fetch('https://flowcrm-production-6d63.up.railway.app/whatsapp/embedded-signup', {
            method: 'POST',
          ...
          .then(function(data) {
            if (data.success) {
              setStatus('¡Conectado! Cerrando...')
              setTimeout(function() {
                window.opener ? window.opener.location.href = redirectBase + '?whatsapp=connected' : null
                window.close()
                window.location.href = redirectBase + '?whatsapp=connected'
              }, 1500)
            } else {
              setStatus('Error: ' + (data.error || 'Intenta de nuevo'), true)
          ...
          setStatus('Cancelado. Puedes intentarlo de nuevo.')
          document.getElementById('connectBtn').disabled = false
        }
      }, {
    ...
    console.error('[WhatsApp connect page] Error:', err.response?.data || err.message)
